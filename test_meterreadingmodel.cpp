...
void TestMeterReadingModel::testSaveToFile() {
	MeterReadingModel model;
	model.addReading(MeterReading("Вода", QDate(2024,1,1), 50.0));
	model.addReading(MeterReading("Газ", QDate(2024,1,2), 100.5));
	...
	QVERIFY(line1.contains("\"Вода\""));
	QVERIFY(line1.contains("2024.01.01"));
	QVERIFY(line1.contains("50"));
	QVERIFY(line2.contains("\"Газ\""));
	QVERIFY(line2.contains("2024.01.02"));
	QVERIFY(line2.contains("100.5"));
}
